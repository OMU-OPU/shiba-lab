...
// --- 第2階層リンククリック時：遷移先が「同じページ」ならメニューを閉じる ---
  $('.g-nav__list_lv2-link').on('click', function() {
    // リンクのパス（/international/など）と現在のページのパスを比較
    var isSamePage = (this.pathname === window.location.pathname);
    var hasHash = (this.hash.length > 0);

    // 「同じパス」かつ「アンカー（#）がついている」場合のみ実行
    if (isSamePage && hasHash) {
      // メニューが開いている状態を示すクラスがあれば解除
      if ($('html').hasClass('js_toggle_open') || $('html').hasClass('js_toggle_open_sp')) {
        
        setTimeout(function(){
          $('html').removeClass('js_toggle_open js_toggle_open_sp');
          $('.g-nav__link_btn').removeClass('g-nav__link_active');
          $('.g-nav__box').removeClass('g-nav__box_active');
          $(".navbar-toggler-text").text("Menu");
          
          // Bootstrapのメニュー（スマホ版）も閉じる
